...
class Ricochet implements Kid {
    ...
    Ricochet() {
        ...
        ballAlbum = new Album("tiles/balls.alb"); //https://lospec.com/palette-list/sweetie-16
    ...
    void draw(PGraphics canvas) {
        //this is so I could edit the sprites and see their updates live (edit sprite > save file > press r to reload)
        if (reloadAlbum) {
            ballAlbum.reload();
            portalAlbum.reload();
            bgdAlbum.reload();
        ...
        for (Arena a : arenas) {
            for (Ball ball : a.balls) {
                ...
                canvas.image(ballAlbum.page(ball.charged ? ball.page + "Charged" : ball.page), -ballAlbum.w / 2, -ballAlbum.h / 2);
                canvas.popMatrix();
            }
        } //end for a
    } //end draw
    ...
    private class Ball {
        ...
        Ball(String page) {
            this.page = page;
            pos = PVector.random2D().mult(50);
            vel = PVector.random2D().mult(50);
            acc = new PVector(0, 0);
            oldPos = new PVector(0, 0);
            portal = null;
            lineTimer = new BoundedInt(20);
            charged = true;
            //make constants?
            mass = 4;
            radius = 10;
    ...
    private class Arena {
        ...
        void update() {
            ...
            // can clamp it here
            int nMaxSimulationSteps = 10;

            // Break up the frame elapsed time into smaller deltas for each simulation update
            float simElapsedTime = edwin.elapsedTime / (float)nSimulationUpdates;
            ...
            for (Ball ball : balls) {
                ...
                || ball.pos.y > HEX_RADIUS) {
                    ...
                    ball.pos.set(0, 0);
                    ball.vel.set(PVector.random2D().mult(20));
                    //println("reset");
                    continue;
                }
                //check for teleport
            ...
            for (int i = 0; i < nSimulationUpdates; i++) {
                for (Ball ball : balls) {
                    ...
                    ball.simTimeRemaining = simElapsedTime; //mess with this to affect play speed
                }
                ...
                for (int j = 0; j < nMaxSimulationSteps; j++) {
                    ...
                    for (Ball ball : balls) {
                        if (ball.simTimeRemaining > 0.0f) {
                            ...
                            ball.acc.set(-ball.vel.x * 0.15, -ball.vel.y * 0.8 + gravity); 
                            //add acceleration to velocity
                            ball.vel.add(ball.acc.copy().mult(ball.simTimeRemaining));                             
                            if (ball.vel.mag() > 1000) ball.vel.setMag(1000); //max speed
                    ...
                    for (Ball ball : balls) {
                        ...
                        for (int c = 0; c < 6; c++) {
                            ...
                            if (fDistance <= ball.radius) {
                                ...
                                fakeball.radius = 1;
                                fakeball.mass = ball.mass * 0.77;
                                fakeball.pos.set(fClosestPointX, fClosestPointY);
                                fakeball.vel.set(-ball.vel.x, -ball.vel.y);
                                if (ball.charged) {
                                    if (fakeball.vel.mag() < 50) fakeball.vel.mult(2.4);
                                    else if (fakeball.vel.mag() < 150) fakeball.vel.mult(2.2);
                                    else if (fakeball.vel.mag() < 300) fakeball.vel.mult(2.0);
                                    else fakeball.vel.mult(1.6);
                                    ball.charged = false;
                                }
...
// for (Ball ball : balls) {
//     //move physics
//     // ball.drag.set(-ball.vel.x * 0.02, -ball.vel.x * 0.02 + min(300, ball.drag.y + 30)); //wind drag and gravity
//     ball.drag.set(-ball.vel.x * 0.1, -ball.vel.y * 0.8 + 120); //wind drag and gravity
//     ball.vel.add(ball.drag.copy().mult(edwin.elapsedTime));
//     ball.pos.add(ball.vel.copy().mult(edwin.elapsedTime));
//     //check for collisions
//     int lastI = 5;
//     for (int i = 0; i < 6; i++) {
//         ball.pos.z = portals[i].z; //hack so I can keep using the portal's z to indicate the arena
//         //ball touches portal
//         if (ball.pos.dist(portals[i]) <= PORTAL_SIZE / 2) { 
//             if (portals[i].z == -1) {
//                 ball.charged = true;
//                 break;
//             }
//             //teleport to that arena
//             int a = (int)portals[i].z;
//             Arena other = arenas[a];
//             other.balls.add(ball);
//             toRemove.add(ball);
//             ball.pos.set(0, 0);
//             break;
//         }
//         //ball touches border
//         if (distanceFromBorder(corners[i], corners[lastI], ball) <= BALL_SIZE / 2) {
//             //displace...
//             float ang = PVector.angleBetween(corners[i], corners[lastI]) - HALF_PI;
//             ball.vel.rotate(PI + ang);
//             //bounce
//             ball.vel.setMag(200);
//             ball.drag.y = -100;
//             if (ball.charged) {
...
// float distanceFromBorder(PVector pt0, PVector pt1, Ball ball) {
//     // https://en.wikipedia.org/wiki/Distance_from_a_point_to_a_line
//     float numerator = (pt1.y - pt0.y) * ball.pos.x - (pt1.x - pt0.x) * ball.pos.y + (pt1.x * pt0.y) - (pt1.y * pt0.x);
//     return abs(numerator) / pt0.dist(pt1);
// }