...
/**
* A bunch of staggered diamonds made with lines
*/
class StarWeb implements Kid {
	...
	StarWeb(PalettePicker picker) {
		...
		clear = commit = false;
		hLineCount = new BoundedInt(0, 20, 7);
		vLineCount = new BoundedInt(0, 20, 7);
		...
		hPadPartial = new BoundedFloat(0, 3.0, 1.0, 0.1);
		vPadPartial = new BoundedFloat(0, 3.0, 1.0, 0.1 );
		hPaletteColor = vPaletteColor = 0;
		perlin = perlinNoise1D(9); //Edwin's version, not Processing's
		perlinIndex = new BoundedInt(0, perlin.length - 1);
		perlinIndex.loops = true;
	}
	...
	void draw(PGraphics canvas) {
		...
		//horizontal lines
		float pad, padPartial, noise = perlin[perlinIndex.increment()] * 50;
		drawOn.stroke(palette.colors.get(hPaletteColor));
		drawOn.strokeWeight(hStrokeWeight.value);
		if (hLineCount.value > 0) drawOn.line(0, center.y, width, center.y);
		for (int i = 1; i < hLineCount.value; i++) {
			pad = hPadding.value * i + noise * i;
			//pad = hPadding.value * i * noise;
			//pad = (i / (float)hLineCount.value) * hPadding.value;
		...
		if (vLineCount.value > 0) drawOn.line(center.x, 0, center.x, height);
		for (int i = 1; i < vLineCount.value; i++) {
			pad = vPadding.value * i + noise * i;
