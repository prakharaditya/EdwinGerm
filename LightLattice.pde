...
public class LightLattice extends DraggableWindow {
	...
	void drawSelf(PGraphics canvas) {
		...
		if (editMode == EDIT_FIGURE) modeFigure.drawSelf(canvas);
		else if (editMode == EDIT_PATH) modePath.drawSelf(canvas);
		else if (editMode == EDIT_PULSE) modePulse.drawSelf(canvas);
		else if (editMode == EDIT_MORPH) modeMorph.drawSelf(canvas);
	...
	*/
	private class LatticePolygon {
		...
		/**
		* We're not implementing Kid but we will follow convention by defining a drawSelf(PGraphics);
		*/
		void drawSelf(PGraphics canvas) {
	...
	*/
	private class ModePulse extends DraggableWindow {
		...
		ModePulse() {
			...
			chargeGrid = new NestedRectBody(body, gridX, gridY + 18, GRID * GRID_W, GRID * GRID_H);
		...
		void drawSelf(PGraphics canvas) {
			...
			figureScale.set(1.0);
			sampleColorPolygon.drawSelf(canvas);
			figureScale.set(s);
			...
			if (chargeGrid.isMouseOver()) {
				int yIndex = (int)((mouseY - chargeGrid.realY()) / GRID);
				int xIndex = (int)((mouseX - chargeGrid.realX()) / GRID);
				canvas.stroke(EdColors.UI_EMPHASIS);
				canvas.noFill();
		...
		String mouse() {
			...
			if (chargeGrid.isMouseOver() && edwin.mouseBtnReleased != 0) {
				//coords
				Integer yIndex = Integer.valueOf((int)(mouseY - chargeGrid.realY()) / GRID);
				Integer xIndex = Integer.valueOf((int)(mouseX - chargeGrid.realX()) / GRID);
